...
##Conclusion: residuals don't follow normal distribution and RSE is 0.4122 which is very large.
...
## We try the Box-Cox power transformation
# eps = 1e-5
# n = 584;
# mu = seq(-min(Affairs$affairsyes) + eps, max(Affairs$affairsyes), length = n)  #assign parameter values
# 
# Affairs['affairsmu'] = Affairs$affairsyes+mu  #Since Y must be positive to perform the boxcox transformation, so add mu to Y
# Trans<-boxcox(affairsmu ~ gendermale + age + yearsmarried + childrenyes + religiousness + education + occupation + rating, data=Affairs,lambda=seq(-3,3, by=0.1))
# 
# lambda<-Trans$x[which.max(Trans$y)]
# lambda
# 
# affairsT<-(Affairs$affairsyes)^lambda   #doesn't make any change for Y due to its 0 and 1 value
# Affairs['affairsT'] = affairsT
# affT<-lm(affairsT ~ gendermale+age+ yearsmarried + childrenyes + religiousness + education + occupation + rating, data=Affairs)
# 
# summary(affT)
# plot(affT, which=2)
##the Box-Cox power transformation doesn't work
...
##Conclusion: much more better now, but still seems strange
##Try to divided the data by Y=1 and Y=0, and see their residuals performance respectively.

no = Affairs[1:451,]   #Create a group called "no" containing all cases reported no extramarital affair
yes = Affairs[452:601,]   #Create a group called "yes" containing all cases reported at least one extramarital affairs

aff.no <- glm(affairsyes~gendermale+age+yearsmarried+childrenyes+religiousness+education+occupation+rating, data=no,family=binomial)
aff.yes <- glm(affairsyes~gendermale+age+yearsmarried+childrenyes+religiousness+education+occupation+rating, data=yes,family=binomial)  #Fit into a logistic regression respectively
...
par(mfrow = c(1,2))
plot(aff.yes, which=2)
plot(aff.yes, which=5)  #Generate qq plot and leverage plot respectively
...
##Conclusion: residuals, both when Y=0 and Y=1, follow normal distribution and has few outliers. Therefore, logistic regression is more suitable.
...
round(cor(Affairs[,1:9]),4) #round correlation matrix to four decimal place
...
##We have a few X outliers and have no Y outliers and influencial points which is good. (Dr. Qin said we can simply neglect those outliers)

```

```{r}

#   (3) Residual (normality/ constant/ independence) check
...
car::vif(glm(occupation~age+gendermale+yearsmarried+childrenyes+religiousness+education+occupation+rating, data=Affairs))
...
##Conclusion: Although the 10-fold cv method selects the 6-predictor model, predicted errors from different models don't have much difference which means the smaller size of predictor will have a economic advantage. Combined with Best subset and Stepwise selection, I chose the 5-predictor model.
##P.S. Since we can't find new data from website to test our model, I asked Dr.Qin whether we can simply use 10-fold cv method to test our model. She said yes.
...
plot(Affairs[,c(1,2,3,6,7,8)])  #create scatter plot
...
#   (2) Residuals(normality/ constant/ independence) check

##Again, divided the data by Y=1 and Y=0, and see their residuals performance respectively.

no = Affairs[1:451,]   #Create a group called "no" containing all cases reported no extramarital affair
yes = Affairs[452:601,]   #Create a group called "yes" containing all cases reported at least one extramarital affairs

aff.red.no <- glm(affairsyes~gendermale + age  + yearsmarried +  religiousness + rating, data=no,family=binomial)
aff.red.yes <- glm(affairsyes~gendermale + age  + yearsmarried +  religiousness + rating, data=yes,family=binomial)  #Fit into a logistic regression respectively

par(mfrow = c(1,2))
plot(aff.red.no, which=2)
plot(aff.red.no, which=5)

par(mfrow = c(1,2))
plot(aff.red.yes, which=2)
plot(aff.red.yes, which=5)  #Generate qq plot and leverage plot respectively
